...
func TestItemPassesFilter(t *testing.T) {
	tests := []struct {
		name, mode, keywords, title string
		want                        bool
	}{
		{"disabled", "none", "spoiler", "Spoiler", true},
		{"block title", "blocklist", "广告\n推广", "这是广告", false},
		{"block case insensitive", "blocklist", "SPONSOR", "A sponsor message", false},
		{"block keeps unmatched", "blocklist", "广告", "Useful article", true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := itemPassesFilter(tt.mode, tt.keywords, tt.title); got != tt.want {
